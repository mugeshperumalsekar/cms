...
import { bank, createDetailsRequest, Regulator, RegulatorListData, RecordTypeData, Dead, InorgPayload, AliasesDTO, AddressDTO, DateOfBirthDTO, DetailsCombineDTO, CreateCountryRegistrationRequest, IdNumberData, CountryHqData, Country, CreateCaseDetailsRequest, createBankDetailsRequests, createCompanyDetailsRequests, createIndPositionsRequests, createIndCaseDetailsRequests, ContactDetailsData, RelativeConfigData, CreateorganizationDetailsRequest } from '../../data/services/Search/search-payload';
...
const Entityview: React.FC = () => {
    ...
    const getidentificationIdaById = (identificationNumId: number) => {
        const country = Idnumber.find((c) => c.id === identificationNumId);
        return country ? country.name : 'Not Available';
    };
    ...
    useEffect(() => {
        const fetchCustomer = async (cmsId: string, uid: string) => {
            try {
                ...
                if (customerData.organizationDetailsData) {
    ...
    const backendColumns = ['Profile', 'Record Type', 'Source Link', 'Regulator', 'Regulator List', 'Name', 'Aliases Name', 'Address', 'Country Details', 'Individual Details', 'Case Details', 'Organization Details', 'Unique Number', "Bank Details"];
    ...
    const renderColumnValue = (columnName: string, DetailsData: any, PartyformData: any) => {
        switch (columnName) {
                                                        ...
                                                        <Table>
                                                            <TableHead>
                                                                <TableRow>
                                                                    ...
                                                                    <TableCell style={tableStyle}><strong>National Identification</strong></TableCell>
                                                                    <TableCell style={tableStyle}><strong>identification Number</strong></TableCell>
                                                            ...
                                                            <TableBody>
                                                                {indOrgformData.indOrgCommonDTO?.map((item, index) => (
                                                                    <TableRow key={index}>
                                                                        ...
                                                                        <TableCell>
                                                                            {item.identificationDTOS?.length > 0 ? (
                                                                                item.identificationDTOS.map((contact, contactIndex) => (
                                                                                    <p key={contactIndex}>{getidentificationIdaById(contact.identificationNumId) || '_'}</p>
                                                                                ))
                                                                            ) : (
                                                                                <p>_</p>
                                                                            )}
                                                                        ...
                                                                        <TableCell>
                                                                            {item.identificationDTOS?.length > 0 ? (
                                                                                item.identificationDTOS.map((contact, contactIndex) => (
                                                                                    <p key={contactIndex}>{contact.identification || '_'}</p>
                                                                                ))
                                                                            ) : (
                                                                                <p>_</p>
                                                                            )}
            ...
            case 'Organization Details':
                return (
                    <>
                        {organization.map((detail, index) => (
                            <Grid item xs={12} sm={3} key={index}>
                                {showFullText ? (
                                    detail.organizationDetails
                                ) : (
                                    detail.organizationDetails.split('\n').map((line, lineIndex) => (
                                        <React.Fragment key={lineIndex}>
                                            {line}
                                            {lineIndex < detail.organizationDetails.split('\n').length - 1 && <br />}
                                        </React.Fragment>
                                    ))
                                )}
                                {detail.organizationDetails.length > MAX_LENGTH && (
